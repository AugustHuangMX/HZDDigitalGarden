...
We start from $a_{t} = \log \tilde{A}$. Since $\rho \in (0,1)$, and $a_{t+1} = \rho a_{t} + \epsilon$, in long run, $a_{t} \to 0$, Thus $\tilde{A} = 1$.


The intuition is to use the guess and verify method. Start by guessing that all variables grow at the same rate $g$. ($G = g+1$)

The key part here is that we have to let $L_{t}$ and $r_{t}$ remain unchanged. 

Reasons:

1. $L_{t} \in [0,1]$.
2. $R_{t} = (1-\alpha) Y_{t}/K_{t}$, which is a ratio. Calder effect
